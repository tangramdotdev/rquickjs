...
macro_rules! loader_impls {
    ...
    (@impl $($t:ident)*) => {
                ...
                $($t: Resolver,)*
            {
                ...
                #[allow(unused_mut)]
                fn resolve<'js>(&mut self, _ctx: &Ctx<'js>, base: &str, name: &str, _attributes: Option<ImportAttributes<'js>>) -> Result<String> {
...
mod test {
    ...
    impl Resolver for IdentityResolver {
        fn resolve<'js>(&mut self, _ctx: &Ctx<'js>, _base: &str, name: &str, _attributes: Option<ImportAttributes<'js>>) -> Result<String> {
            Ok(name.into())
        }
    }
